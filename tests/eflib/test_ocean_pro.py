...
from custom_components.ef_ble.eflib.devices.ocean_pro import (
    CircuitStatus,
    Device,
    GridStatus,
)
...
def test_ocean_pro_recognizes_hr61_and_hr51():
    # HR61 (encrypt_type 0) and HR51 ("E7 P2S", encrypt_type 7) are both OCEAN Pro.
    assert Device.check(b"HR61XXXXXXXXX001")
    assert Device.check(b"HR51XXXXXXXXX001")
    assert not Device.check(b"HR62XXXXXXXXX001")  # SHP3, not OCEAN Pro
...
async def test_ocean_pro_decodes_circuit_status_and_names(device, packet_sequence):
...
async def test_ocean_pro_set_circuit_power_gangs_split_phase(device, packet_sequence):
...
async def test_ocean_pro_registers_userid_once_on_time_request(device, packet_sequence):
