...
async def test_shp3_set_backup_reserve_level_sends_config_write(device):
    ...
    assert (packet.src, packet.dst, packet.cmdSet, packet.cmdId) == (
        0x21,
        0x60,
        0xFE,
        0x11,
...
async def test_shp3_config_write_mirrors_post_frame(device, packet_sequence):
    ...
    assert (sent.src, sent.dst, sent.cmdSet, sent.cmdId) == (
        post.src,
        post.dst,
        post.cmdSet,
        post.cmdId,
    ...
    decoded = PacketV4.fromBytes(sent.toBytes())
...
async def test_shp3_registers_userid_once_on_time_request(device, packet_sequence):
    ...
    userid = [
        ...
        if (c.args[0].cmdSet, c.args[0].cmdId) == (0x35, 0xA8)
    ]
    assert len(userid) == 1
