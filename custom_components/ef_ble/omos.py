...
from .eflib.exceptions import AuthErrors, FailedToAuthenticate
...
async def connect_omos(
    ...
    *,
    ...
    try:
        ...
        except (AuthErrors.NeedRefreshToken, FailedToAuthenticate) as err:
            # A freshly minted token still failed to authenticate, so retrying setup
            # would just re-hit the token service on every backoff. Escalate to reauth
            # (which does not auto-retry) so the user resolves the account/device pairing.
            raise AuthErrors.NeedRefreshToken(
                "OMOS token refresh did not resolve authentication"
            ) from err
